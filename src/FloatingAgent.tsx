...
export default function FloatingAgent({ capture, state, children }: {
  ...
  async function open() {
    ...
    if (!api) {
      setNotice("La miniatura está disponible dentro de esta página. Para verla sobre otras aplicaciones, usa un navegador con ventana flotante compatible.");
      return;
    }
    ...
    try {
      ...
      if (alive.current) setNotice("No se pudo abrir la ventana externa. Puedes usar la miniatura aquí o volver a intentarlo.");
    } finally {
  ...
  const panel = (
    <section className={`floating-agent ${pip ? "is-external" : "is-docked"}`} aria-label="Miniatura del agente">
      ...
      <div className="floating-agent-identity">
        <svg className="model-avatar" viewBox="0 0 100 100" role="img" aria-label="Avatar del agente Cognitive">
          <circle cx="50" cy="50" r="48" fill="#e3eee7" />
          <path d="M50 18v10M45 18h10" stroke="#39715d" strokeWidth="4" strokeLinecap="round" />
      ...
      <div className="floating-agent-controls">
        <button type="button" className={microphone === "on" ? "mic-active" : ""} disabled={!active || microphone === "requesting" || phase === "stopping" || (microphone === "off" && phase !== "sharing")} aria-pressed={microphone === "on"} onClick={() => microphone === "off" ? void capture.enableMicrophone() : capture.toggleMicrophone()}>
          <Icon name="mic" size={20} />{microphone === "on" ? "Silenciar" : microphone === "muted" ? "Activar voz" : microphone === "requesting" ? "Permiso…" : "Micrófono"}
        </button>
        {phase === "sharing" && <button type="button" disabled={microphone === "requesting"} onClick={() => capture.record()}><Icon name="record" />Grabar</button>}
        {phase === "recording" && <button type="button" onClick={() => capture.pause()}><Icon name="pause" />Pausar</button>}
